...
def scrape_novels(pageNo):
    with sync_playwright() as p:
        ...
        try:
            ...
            titles = page.query_selector_all(".search_title")
            print(f"Found {len(titles)} novels on Page {pageNo}:")

            for title in titles:
                novel_title = title.inner_text().strip()
                all_novels.append(novel_title)
